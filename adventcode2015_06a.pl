...
sub operate($) {
  ...
  if ( $op =~ m/^turn on (\d+),(\d+) through (\d+),(\d+)$/ ) {
    ...
    main::deblog("turned on " . (($xmax-$xmin+1)*($ymax-$ymin+1)) . " lights: $xmin,$ymin : $xmax,$ymax");
    ...
    main::deblog("turned off " . (($xmax-$xmin+1)*($ymax-$ymin+1)) . " lights: $xmin,$ymin : $xmax,$ymax");
  } elsif ($op =~ m/^toggle (\d+),(\d+) through (\d+),(\d+)$/) {
    # toggle 0,0 through 999,0
    my ($xmin, $ymin, $xmax, $ymax) = ($1, $2, $3, $4);
    for(my $i = $xmin; $i <= $xmax; $i++) {
      for(my $j = $ymin; $j <= $ymax; $j++) {
        $lights_mat[$i][$j] = !$lights_mat[$i][$j];
      }
    }
    main::deblog("toggled " . (($xmax-$xmin+1)*($ymax-$ymin+1)) . " lights: $xmin,$ymin : $xmax,$ymax");
  } else {
...
$algo->operate("turn on 0,0 through 999,999");
$result = $algo->exec();
ok( $result == 1000000, "shine all lights; $result" );

$algo->operate("toggle 0,0 through 999,0");
$result = $algo->exec();
ok( $result == 1000000-1000, "shine all but first row via toggling; $result" );

$algo->operate("toggle 0,0 through 999,0");
$result = $algo->exec();
ok( $result == 1000000, "reshine all via toggling; $result" );

$algo->operate("turn off 499,499 through 500,500");
$result = $algo->exec();
ok( $result == 1000000-4, "now; $result" );

$algo->operate("toggle 0,0 through 1,1");
$result = $algo->exec();
ok( $result == 1000000-4-4, "now; $result" );

$algo->operate("turn on 0,1 through 1,1");
$result = $algo->exec();
ok( $result == 1000000-4-4+2, "now; $result" );
